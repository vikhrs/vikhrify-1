...
app.post('/api/auth', async (req, res) => {
    ...
    if (!/^[a-zA-Z0-9_]+$/.test(username)) return res.status(400).send();
    if (type === 'reg') {
...
app.post('/api/post', async (req, res) => { 
    db.posts.push({author: req.body.username, text: req.body.text});
    let u = db.users.find(x => x.username == req.body.username);
    if(u) u.rating += 1;
    await save(); res.json({ok:1}); 
});
app.listen(3000);
